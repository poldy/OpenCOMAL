...
/*
 * Like strncpy, but guarantees that "dest" will be NUL-terminated. Copied 
...
main(int argc, char *argv[])
{
    ...
    if (inclfile) {
        ...
        mname = argv[1];
        if ((int) strlen((t = strrchr(mname, '/')) ? t + 1 : mname) >
            MDIRSIZ) {
        ...
        if ((outfp = fopen(outname, "w")) == NULL) {
            ...
            for (cp = mname; *cp; cp += i) {
                ...
                if (i < 0) {
                    fprintf(stderr,
                            "mkcatdefs: filename contains invalid character\n");
                    exit(1);
                }
                if (i == 1) {
                    if (islower(*cp) != 0)
        ...
        term_strncpy(outname, "msg.h", PATH_MAX);
    ...
    /*
     * if message descriptor files were specified then process each one in 
     * turn 
     */
     ...
     /*---- fname: message descriptor file name ----*/
{
    ...
    while (fgets(line, MAXLINELEN, descfile)) {
        line[MAXLINELEN - 1] = '\0';    /* terminate in case length
                                         * exceeded */
        /*
            ...
            if (sscanf(cp, "%63s", msgname) && msgname[0]) {
                ...
                if (len == 1 && isalpha(*cp) != 0) {
                    ...
                    if ((n >= msgno) || (n == 0 && msgno == 1))
                        ...
                        if (n == 0)
                            fprintf(stderr,
                                    "mkcatdefs: %d is an invalid identifier\n",
                                    msgno);
                        else if (n == msgno)
                            fprintf(stderr,
...
chkcontin(char *line)
{
    ...
    wchar_t         wc;         /* process code of current character in
                                 * line */
    wchar_t         wcprev;     /* process code of previous character in
                                 * line */
...
insert(char *tname, int seqno)
     /*
      * tname - pointer to symbol seqno - integer value of symbol 
      */
{
    struct name    *ptr,
                   *optr;
    ...
    if (rslt == 0)              /* found the symbol already defined */
        return (-1);
    else {                      /* symbol not defined yet so put it into
                                 * symbol table */
