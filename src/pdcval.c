...
PUBLIC void
val_print(int stream, void *result, enum VAL_TYPE type)
{
    char            buf[64];
    char           *pptr = buf;
    long            len = -1L;
    ...
    if (len)
        my_put(stream, pptr, len);
}


PUBLIC void
val_copy(void *to, void *from, enum VAL_TYPE ttype, enum
         VAL_TYPE ftype)
{
    if (ttype == V_STRING) {
        if (ftype != V_STRING)
            run_error(VALUE_ERR, "Wrong type (must be string)");

        if (*(struct string **) to)
            mem_free(*(struct string **) to);

        *(struct string **) to =
            (struct string *) str_dup(RUN_POOL, (struct string *) from);
        ...
        if (ftype == V_FLOAT)
            ...
            *(double *) to = *(long *) from;
            ...
            run_error(VALUE_ERR, "Wrong type (must be numeric)");
    else if (ttype == V_INT)
        if (ftype == V_INT)
            *(long *) to = *(long *) from;
...
PUBLIC void
val_free(void *result, enum VAL_TYPE type)
{
    if (type == V_STRING)
        mem_free(result);
...
PUBLIC double
val_double(struct expression *exp)
{
    void           *result;
    enum VAL_TYPE   type;
    double          d = 0.0;
...
PUBLIC int
val_cmp(int op, void *r1, void *r2, enum VAL_TYPE t1, enum
        VAL_TYPE t2)
{
    double          d1,
                    d2;
...
PUBLIC void
val_neg(void *value, enum VAL_TYPE type)
{
    if (type == V_INT)
...
PUBLIC long    *
val_int(long i, void *ptr, enum VAL_TYPE *type)
{
...
PUBLIC double  *
val_float(double f, void *ptr, enum VAL_TYPE *type)
{
...
PUBLIC void
val_intadd(long *v1, long *v2, void **result, enum VAL_TYPE
           *type)
{
    int             minv1 = *v1 < 0;
    int             minv2 = *v2 < 0;
    ...
    *type = V_INT;
    ...
    if (do_float) {
        *result = val_float((double) *v1 + (double) *v2, NULL, type);
        cell_free(v1);
    } else
        *v1 = v3;
...
PUBLIC void
val_intsub(long *v1, long *v2, void **result, enum VAL_TYPE
           *type)
{
    *v2 = -*v2;
...
PUBLIC void
val_intmul(long *v1, long *v2, void **result, enum VAL_TYPE
           *type)
{
    long            v3 = *v1 * *v2;
...
PUBLIC void
val_intdiv(long *v1, long *v2, void **result, enum VAL_TYPE
           *type)
{
    if (*v2 == 0)
...
PUBLIC long
val_mustbelong(void *value, enum VAL_TYPE type, int freeit)
{
    long            n = 0;
