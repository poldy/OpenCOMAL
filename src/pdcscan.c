...
enum scan_entry_special { NOT_SPECIAL, SHORT_FORM, EXIT, PROCFUNC,
    EPROCFUNC,
...
PRIVATE void
scan_stack_push(int sym, struct comal_line *line)
{
...
PRIVATE void
scan_stack_peek(int *sym, struct comal_line **line)
{
    if (!scan_sp) {
        *sym = -1;
        *line = NULL;
...
PRIVATE void
scan_stack_pop(int *sym, struct comal_line **line)
{
...
PRIVATE void
scan_stack_search(int sym1, int sym2, int sym3, int *result,
...
PRIVATE struct comal_line *
routine_search(struct id_rec *id, int type, struct comal_line *curproc)
{
 ...
 */
PRIVATE bool
scan_pass2(struct seg_des *seg, char *errtxt, struct comal_line **errline)
{
...
PRIVATE bool
scan_pass4(struct seg_des *seg, char *errtxt, struct comal_line **errline)
{
    struct comal_line *curline;
        ...
        if (theline->cmd == procSYM || theline->cmd == funcSYM
            ...
            *errline = curline;
            ...
            if (!procline)      // || !procline->lc.pfrec.closed)
                err = "IMPORT only allowed within PROCs/FUNCs or MODULEs";
        } else if (theline->cmd == localSYM) {
            if (!procline || procline->lc.pfrec.closed)
                err = "LOCAL only allowed within open PROCs/FUNCs";
        } else if (theline->cmd == staticSYM) {
            if (!procline || procline->cmd == moduleSYM)
                err = "STATIC only inside a PROC or FUNC please";
        } else if (theline->cmd == returnSYM) {
            if (!procline)
                err = "Can only RETURN from inside a PROC, FUNC or MODULE";
            else if (procline->cmd == funcSYM) {
                if (!theline->lc.exp)
                    err =
                        "This statement (inside a FUNC) must RETURN an expression";
                else if (!type_match1
                         (procline->lc.pfrec.id, theline->lc.exp))
                    err =
            ...
            if (!routine_search(proccall->id, procSYM, procline)) {
                procfound = 0;
                ...
                if (procline) {
                    ...
                    while (!procfound && walkline) {
                        pwalk = walkline->lc.pfrec.parmroot;

                        while (pwalk && pwalk->id != proccall->id)
                            pwalk = pwalk->next;

                        procfound = (pwalk && pwalk->ref == procSYM);

                        if (!sp)
                            walkline = NULL;
                        else {
                            sp--;
                            walkline = scan_stack[sp].line;
                        }
                    }
                }

                if (!procfound) {
                    Fmt_sfmt(errtxt, MAX_LINELEN, "PROCedure %s not found",
                             proccall->id->name);
                    *errline = curline;
                    return false;
                }
            ...
            if (seg) {
                ...
                err = "RESTORE without any DATA line in program";
            else if (theline->lc.id) {
                walk = curenv->progroot;

                while (walk
                       && !(walk->cmd == idSYM
                            && walk->lc.id == theline->lc.id))
                    walk = walk->ld->next;
                ...
                if (!walk)
                    ...
                    if (!walk)
                        err = "No DATA statements follow the label";
                }
...
PRIVATE bool
check_case(struct comal_line *fromline, char *errtxt,
           struct comal_line **errline)
{
    ...
    if (fromline->cmd != whenSYM) {
        *errline = fromline;
        term_strncpy(errtxt, "CASE should be followed by WHEN",
                     MAX_LINELEN);
        return false;
    }
...
PRIVATE bool
do_special1(struct scan_entry *p, struct comal_line *theline,
            struct comal_line *curline,
            struct comal_line **errline, char *errtxt,
            int *level, struct comal_line **procroot, struct seg_des *seg)
{
    int             sym;
    ...
    case EXIT:
        ...
        if (sym == 0) {
            term_strncpy(errtxt, "Can only EXIT from within a loop",
                         MAX_LINELEN);
            *errline = curline;
            return false;
        }
    ...
    case RETRY:
        ...
        if (sym == 0) {
            term_strncpy(errtxt,
                         "Can only RETRY from within the HANDLER part of a TRAP/ENDTRAP",
                         MAX_LINELEN);
            *errline = curline;
            return false;
        }
    ...
    case PROCFUNC:
        if (routine_search_horse
            ...
            Fmt_sfmt(errtxt, MAX_LINELEN, "%s %s multiply defined",
                     lex_sym(theline->cmd), theline->lc.pfrec.id->name);
            *errline = curline;
            return false;
        }
        ...
        for (i = scan_sp - 1; i >= 0; i--) {
             ...
             */
            if (sym != 0 && sym != endprocSYM && sym != endfuncSYM
                && sym != endmoduleSYM) {
                term_strncpy(errtxt, "PROC/FUNC may not be defined here",
                             MAX_LINELEN);
                *errline = curline;
                return false;
            }
        ...
        scan_stack_search(endprocSYM, endfuncSYM, endmoduleSYM, &sym,
                          &lineptr);
...
PUBLIC bool
scan_scan(struct seg_des *seg, char *errtxt, struct comal_line **errline)
{
    struct comal_line *curline;
    struct comal_line *theline;
    struct comal_line *lineptr;
    ...
    *errtxt = 0;
    ...
    while (curline) {
        ...
        if (p->sym) {
            if ((p->special == SHORT_FORM && cmd2) ||
                (p->special == PROCFUNC && theline->lc.pfrec.external)
                || (theline->cmd == trapSYM && theline->lc.traprec.esc))
            ...
            if (p->atindent0 && scan_sp != 0) {
                Fmt_sfmt(errtxt, MAX_LINELEN, "Can not have %s here",
                         lex_sym(p->sym));
                *errline = curline;
                return false;
            }
            ...
            if (!skip_processing) {
                if (p->expectsym1 || p->expectsym2) {
                    ...
                    if (sym != p->expectsym1 && sym != p->expectsym2) {
                        if (sym != -1)
                            Fmt_sfmt(errtxt,
                                     MAX_LINELEN,
                                     "%s expected, not %s",
                                     lex_sym(sym), lex_sym(theline->cmd));
                        else
                            Fmt_sfmt(errtxt,
                                     MAX_LINELEN,
                                     "Unexpected %s",
                ...
                if (p->leavessym)
                    scan_stack_push(p->leavessym, curline);
            }
        }
    ...
    if (scan_sp) {
        Fmt_sfmt(errtxt, MAX_LINELEN,
                 "%d open control structure%s at EOP\n", scan_sp,
                 scan_sp == 1 ? "" : "s");
        return false;
    }
...
prog_structure_scan(void)
{
    ...
    while (curline) {
        ...
        if (p->sym) {
            if ((p->special == SHORT_FORM && cmd2) ||
                (p->special == PROCFUNC && curline->lc.pfrec.external)
                || (curline->cmd == trapSYM && curline->lc.traprec.esc))
                skip_processing = 1;    /* Prevent indention */
...
PUBLIC int
scan_necessary(struct comal_line *line)
{
    int             i;
    int             cmd2 = line_2cmd(line);

    for (i = 0; scan_tab[i].sym; i++)
        if (scan_tab[i].sym == line->cmd || scan_tab[i].sym == cmd2) {
            ...
            if (scan_tab[i].leavessym)
                if (line_2cmd(line) != 0
                    ||
                    ((line->cmd == procSYM
                      || line->cmd == funcSYM || line->cmd == moduleSYM)
                     && line->lc.pfrec.external)
                    || (line->cmd == trapSYM && line->lc.traprec.esc))
                    return NO_STRUCTURE;
                else
                    return STRUCTURE_START;
            else if (scan_tab[i].expectsym1 || scan_tab[i].expectsym2)
...
PUBLIC bool
assess_scan(struct comal_line *line)
{
    const char     *msg = NULL;
    ...
    if (line == curenv->curline) {
        msg =
            catgets(catdesc, ScanSet, ScanCurExec,
                    "the current execution line");
    } else if (line == curenv->datalptr) {
        msg =
            catgets(catdesc, ScanSet, ScanCurData,
                    "the current DATA line");
    } else if (scan_necessary(line)) {
        msg =
            catgets(catdesc, ScanSet, ScanStrucLine,
                    "a program structure line");
    }
    if (msg) {
        curenv->scan_ok = false;
        ...
        if (curenv->running == HALTED && !curenv->con_inhibited) {
            my_printf(MSG_DIALOG, true,
                      catgets(catdesc, ScanSet, ScanInhibCon,
