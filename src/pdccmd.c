...
PRIVATE void
cmd_list_horse(struct string *filename, long from, long to)
{
...
PRIVATE bool
cmd_list(struct comal_line *line)
{
    ...
    if (!line->lc.listrec.id)
        ...
        while (curline
               &&
               !((curline->cmd == procSYM
                  || curline->cmd == funcSYM || curline->cmd == moduleSYM)
...
PRIVATE bool
cmd_enter(struct comal_line *line)
{
    FILE           *yyenter;
    ...
    while (!stoppen) {
        ...
        if (stoppen) {
            ...
            if (!aline) {
                my_nl(MSG_DIALOG);
                my_printf(MSG_DIALOG, true, "Ignored line: %s", tline);
...
PRIVATE bool
cmd_new(struct comal_line *line __my_unused)
{
    if (check_changed()) {
        prog_new();
        mem_freepool(PARSE_POOL);
...
PUBLIC bool
cmd_scan(struct comal_line *line __my_unused)
{
    prog_total_scan();
    return false;
}
...
PRIVATE bool
cmd_save(struct comal_line *line)
{
    if (!line->lc.str && !curenv->name)
        ...
        if (line->lc.str) {
            ...
            curenv->name = my_strdup(MISC_POOL, line->lc.str->s);
        } else
...
PRIVATE bool
cmd_load(struct comal_line *line)
{
    char           *fn;
...
PRIVATE bool
cmd_auto(struct comal_line *line)
{
    char            buf[MAX_LINELEN];
    char           *buf2;
    int             direct_cmd = 0;
...
PRIVATE bool
cmd_cont(struct comal_line *line __my_unused)
{
...
PRIVATE bool
cmd_run(struct comal_line *line __my_unused)
{
    mem_freepool(PARSE_POOL);
...
PRIVATE bool
cmd_del(struct comal_line *line)
{
    long            from = line->lc.twonum.num1;
    long            to = line->lc.twonum.num2;

    if (from == 0 && to == INT_MAX)
        run_error(CMD_ERR, "Please mention a line number range with DEL");
...
PRIVATE bool
cmd_edit(struct comal_line *line)
{
    char            buf[MAX_LINELEN];
    char           *buf2;
    long            nr = line->lc.twonum.num1;
...
PRIVATE void
renum_horse(long num, long step)
{
    struct comal_line *work = curenv->progroot;
...
PRIVATE bool
cmd_renumber(struct comal_line *line)
{
...
PRIVATE bool
cmd_quit(struct comal_line *line __my_unused)
{
...
PRIVATE bool
cmd_env(struct comal_line *line)
{
    if (line->lc.id == NULL)
        cmd_env_list();
    else
...
PUBLIC bool
cmd_exec(struct comal_line *line, bool *result)
{
    int             i;
