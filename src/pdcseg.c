...
PUBLIC void
seg_total_scan(struct seg_des *seg)
{
...
PRIVATE void
seg_proccheck(struct comal_line *mline, struct comal_line
              *sline)
{
    ...
    if (mline->cmd != sline->cmd)
        run_error(EXT_ERR, "PROC/FUNC mismatch in external segment");

    if (mpf->id != spf->id)
        run_error(EXT_ERR, "EXTERNAL PROC/FUNC not in external segment");
...
PUBLIC struct seg_des *
seg_static_load(struct comal_line *line)
{
...
PUBLIC struct seg_des *
seg_dynamic_load(struct comal_line *line)
{
...
PUBLIC struct seg_des *
seg_static_free(struct seg_des *seg)
{
...
PUBLIC struct seg_des *
seg_dynamic_free(struct seg_des *seg)
{
