...
PUBLIC void
my_put(int stream, const char *buf, long len)
{
    if (sel_outfile && stream == MSG_PROGRAM) {
...
PUBLIC void
my_printf(int stream, bool newline, const char *s, ...)
{
...
PUBLIC bool
exp_list_of_nums(struct exp_list *root)
{
    while (root) {
...
check_changed(void)
{
    ...
    return sys_yn(MSG_DIALOG,
                  catgets(catdesc, MiscSet, MiscNotSaved,
                          "Latest changes have not yet been saved! Proceed? "));
}

PUBLIC bool
check_changed_any(void)
{
    ...
    while (walk) {
        if (walk->env->changed) {
            my_printf(MSG_DIALOG, true,
                      catgets(catdesc, MiscSet, MiscEnvUnsaved,
    ...
    if (any_changes) {
        result =
            sys_yn(MSG_DIALOG,
                   catgets(catdesc, MiscSet, MiscProceed, "Proceed? "));
    }
    return result;
}
...
PUBLIC void
puts_line(int stream, struct comal_line *line)
{
    char            buf[MAX_LINELEN];
    char           *buf2;
...
PUBLIC int
type_match1(struct id_rec *id, struct expression *exp)
{
...
exp_of_id(struct expression *exp)
{
    struct id_rec  *id = NULL;

    IP(exp->optype == T_EXP_IS_NUM
       || exp->optype == T_EXP_IS_STRING, "Exp_id() internal error #1");
...
PUBLIC bool
exp_of_string(struct expression *exp)
{
...
PUBLIC void
give_run_err(struct comal_line *line)
{
    if (curenv->error) {
        if (line)
...
PUBLIC void
check_lval(struct expression *exp)
{
    struct sym_item *sym;
    ...
    if (exp->optype != T_ID && exp->optype != T_SID
        && exp->optype != T_ARRAY && exp->optype != T_SARRAY)
...
PUBLIC int
proclevel(struct comal_line *proc)
{
    int             i;
...
PUBLIC struct comal_line *
line_2line(struct comal_line *line)
{
    switch (line->cmd) {
    case forSYM:
...
PUBLIC int
line_2cmd(struct comal_line *line)
{
...
PUBLIC struct comal_line *
stat_dup(struct comal_line *stat)
{
...
PUBLIC void
remove_trailing(char *s, const char *trailer, const char *subst)
{
    int             l = strlen(s);
