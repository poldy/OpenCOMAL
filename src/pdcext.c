...
PUBLIC int
ext_sys_exp(struct exp_list *exproot, void **result, enum
            VAL_TYPE *type)
{
    char           *cmd;
    ...
    if (strcmp(cmd, "version") == 0) {
        if (exproot)
            run_error(SYS_ERR, "SYS(version) takes no parameters");
...
PUBLIC int
ext_syss_exp(struct exp_list *exproot, struct string
             **result, enum VAL_TYPE *type)
{
    ...
    if (strcmp(cmd, "host") == 0) {
        if (exproot)
            run_error(SYS_ERR, "SYS$(host) takes no parameters");

        s = HOST_OS;
    } else if (strcmp(cmd, "interpreter") == 0) {
        if (exproot)
            run_error(SYS_ERR, "SYS$(interpreter) takes no parameters");
...
push_inpfile(void)
{
    struct inpfile_stkent *work =
        (struct inpfile_stkent *) mem_alloc(MISC_POOL, sizeof(*work));
...
PUBLIC int
ext_sys_stat(struct exp_list *exproot)
{
    struct string  *name;
    enum VAL_TYPE   type;
    bool           *flag;
    ...
    if (strcmp(cmd, "sysin") == 0) {
        if (!exproot->next)
            run_error(SYS_ERR, "Too few parameters for SYS sysin");

        if (exproot->next->next)
            run_error(SYS_ERR, "Too much parameters for SYS sysin");

        calc_exp(exproot->next->exp, (void **) &name, &type);

        if (type != V_STRING)
            run_error(TYPE_ERR, "Sysin parameter not a string");
        ...
        if (name->len == 0)
            ...
            if (!sys_inpfile) {
                pop_inpfile();
                run_error(SYS_ERR,
                          "Error opening sysin: %s", strerror(errno));
            }
        }
        ...
        if (!exproot->next)
            run_error(SYS_ERR, "Too few parameters for SYS sysout");

        if (exproot->next->next)
            run_error(SYS_ERR, "Too much parameters for SYS sysout");
        ...
        if (exproot->next)
            run_error(SYS_ERR, "Too much parameters for SYS memdump");
...
PUBLIC bool
ext_get(int stream, char *line, int maxlen, const char *prompt)
{
    line[0] = '\0';
    ...
    if (sys_inpfile) {
        ...
        if (!eof)
            run_error(SYS_ERR, "Error reading sysin: %s", strerror(errno));
